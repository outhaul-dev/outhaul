...
func (s *Store) DeleteApp(ctx context.Context, id int64) error {
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM attachments WHERE app_id = ?`, id); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM preview_configs WHERE app_id = ?`, id); err != nil {
		return err
	}
